  ...
  <div class="space-y-6">
    ...
    >
      <template #actions>
        ...
        <button class="btn-primary shrink-0 bg-teal-500 hover:bg-teal-600 sm:w-auto" type="button" @click="openDrawer()">
          <Plus class="size-4" />
          新增特产商品
        </button>
      </template>
      ...
      <div class="space-y-6">
        <DataTable :columns="['排序', '商品', '分类 / 产地', '标签', '评论', '状态', '操作']">
          ...
          >
            ...
            <td class="px-4 py-4">
              <span class="inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-extrabold text-emerald-700 ring-1 ring-emerald-200">
                {{ product.tag }}
              </span>
            </td>
