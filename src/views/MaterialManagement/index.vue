...
<template>
  <div class="space-y-6">
    <PageToolbar title="素材管理" description="商品封面、Banner、详情图、评价图、地区图片分组管理，后续直接绑定到内容或商品。" search-placeholder="搜索素材">
      <template #actions>
        <button class="btn-primary shrink-0" type="button" @click="uploadOpen = true">
          <Plus class="size-4" />
          上传素材
        </button>
      </template>
    ...
    <div v-if="uploadOpen" class="fixed inset-0 z-50 grid place-items-center bg-slate-950/35 p-4" @click.self="uploadOpen = false">
      <section class="w-full max-w-lg rounded-lg bg-white shadow-2xl">
        ...
        <footer class="flex justify-end gap-3 border-t border-slate-200 p-5">
          <button class="btn-secondary" type="button" @click="uploadOpen = false">取消</button>
          <button class="btn-primary" type="button" @click="uploadOpen = false">
            <Upload class="size-4" />
            确认上传
          </button>
        </footer>
