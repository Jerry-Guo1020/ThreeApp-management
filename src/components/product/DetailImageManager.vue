  ...
  <section class="card p-5">
    ...
    <div class="mt-5 rounded-lg border border-slate-200 bg-white p-4">
      ...
      <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        >
          <div
            class="h-24 rounded-lg bg-gradient-to-br"
          ...
          <p class="mt-2 break-all text-xs text-slate-400">{{ image.url || '未设置图片地址' }}</p>
...
async function handleSave() {
  const result = await saveProductDetailImages(localProduct.value.id)
